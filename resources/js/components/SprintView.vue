...
<template>
    <section class="sprint-view" aria-labelledby="sprint-view-title">
        <SprintHeader :sprint="sprint" :is-refreshing="isLoading" @refresh="loadSprint" />

        <div class="sprint-content" aria-live="polite">
            <div v-if="isLoading" class="sprint-state sprint-loading" role="status">
                <span class="spinner spinner-dark" aria-hidden="true"></span>
                Loading active sprint…
            </div>

            <div v-else-if="error" class="sprint-state status-message status-error" role="alert">
            ...
            <div v-else-if="!sprint" class="sprint-state sprint-empty">
                <h2 id="sprint-view-title">No active sprint found.</h2>
                <p>Refresh after an active sprint becomes available in Jira.</p>
            </div>

            <div v-else-if="issues.length === 0" class="sprint-state sprint-empty">
                <h2 id="sprint-view-title">No subtasks assigned to you in the active sprint.</h2>
